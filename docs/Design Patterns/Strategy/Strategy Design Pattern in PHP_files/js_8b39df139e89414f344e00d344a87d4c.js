...
var hljs = new function() {
  ...
  var DEFAULT_LANGUAGES = ['python', 'ruby', 'perl', 'php', 'css', 'xml', 'html', 'django', 'javascript', 'actionscript', 'java', 'cpp', 'c-sharp', 'sql', 'smalltalk', 'ini', 'diff'];
  var ALL_LANGUAGES = (DEFAULT_LANGUAGES.join(',') + ',' + ['1c', 'axapta', 'delphi', 'output', 'rib', 'rsl', 'vbscript', 'profile', 'dos', 'bash'].join(',')).split(',');
  var LANGUAGE_GROUPS = {
    'xml': 'www',
    'html': 'www',
    'css': 'www',
...
$(document).ready(function(){
  text = $('.putanchors');
  var i = 0;
  $('.putanchors > p, .putanchors > table, .putanchors > ul, .putanchors > ol, .putanchors > pre, .putanchors > blockquote, .putanchors > div[class*="paragraph"], .putanchors > div[class*="examples"]').addClass('addanchor');
  $('.addanchor', text).each(function() {
    i = i + 1;
    $(this).before('<a id="permalink-' + i + '" href="#permalink-' + i + '" title="' + Drupal.t('Permalink to this place') + '" class="permalink">' + PadDigits(i, 2) + '</a>');
  });
  ...
  $('a[href*=#]').bind('click', function() {
    if ($(this).attr('href') != '#' && location.pathname.replace(/^\//,'') == this.pathname.replace(/^\//,'') && location.hostname == this.hostname) {
...
function goAnchor(id) {
  ...
  offset = $('#' + id).offset();
  if (offset != undefined) {
    $('html').animate({scrollTop: offset.top - height}, 'fast');
  }
}
...
/*
  // add anchors
  $finder = "/(<table([^>]*?)>(.*?)<\/table>|<table([^>]*?)>(.*?)<\/table>|<p([^>]*?)>(.*?)<\/p>|<ul([^>]*?)>(.*?)<\/ul>|<ol([^>]*?)>(.*?)<\/ol>|<pre><code([^>]*?)>(.*?)<\/code><\/pre>)/is";
  $text = preg_replace_callback($finder, _bonus_filter_add_anchor, $text);

function _bonus_filter_add_anchor($matches) {
  static $id = 0;
  $id++;
  if (strpos($matches[2], 'noanchor') || strpos($matches[4], 'noanchor') || strpos($matches[6], 'noanchor')) {
    return $matches[1];
  }
  return '<a id="permalink-'.$id.'" href="#permalink-'.$id.'" title="'.t('Permalink to this place').'" class="permalink">'.sprintf("%02d", $id).'</a>'.$matches[1].'';
}


str.replace(REGEXP, function(arguments){return window[cbck](arguments);});*/;
