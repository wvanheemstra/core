/**
 * The event slide mediator essentially fulfils the passive view pattern for the event slide view.
 */
Ext.define("Core.mediator.touch.event.slide.Mediator", {
    ...
    setupGlobalEventListeners: function() {
        ...
        this.eventBus.addGlobalEventListener(Core.event.event.Event.GET_EVENT_SLIDE_SUCCESS, this.onGetEventSlideSuccess, this);
        this.eventBus.addGlobalEventListener(Core.event.event.Event.GET_EVENT_SLIDE_FAILURE, this.onGetEventSlideFailure, this);
    },

    /**
     * Dispatches the application event to get the slide of events.
     */
    getEventSlideData: function() {
        this.logger.debug("getEventSlideData");
        this.getView().setMasked({
            xtype: "loadmask",
            message: nineam.locale.LocaleManager.getProperty("eventSlide.loading")
        });
        var evt = Ext.create("Core.event.event.Event", Core.event.event.Event.GET_EVENT_SLIDE);
        this.eventBus.dispatchGlobalEvent(evt);
    },

    /**
     * Handles the show event detail event from the event slide view. Grab the data model
	...
	showEventModal: function(args) {
		...
		Core.config.event.Config.setPreviousView('eventslide');
		this.navigate(Core.event.navigation.Event.ACTION_SHOW_EVENT_MODAL);
	},
    ...
    /**
     * Handles the login success application-level event. Slide the event slide view
     * onto stage.
     */
    onLoginSuccess: function() {
        this.logger.debug("onLoginSuccess");        
		if(Core.config.event.Config.getNextView()==='eventslide') {
        	this.navigate(Core.event.authentication.Event.LOGIN_SUCCESS);
        	//this.getEventSlideData(); // TEMP commented out by wvh
     ...
     */
    onGetEventSlideSuccess: function() {
        this.logger.debug("onGetEventSlideSuccess");
        this.getView().setMasked(false);
        this.getSlide().setStore(this.eventStore);
    },

    /**
     * Handles the get events failure event from the login controller.
     */
    onGetEventSlideFailure: function() {
        this.logger.debug("onGetEventSlideFailure");
        this.getView().setMasked(false);
    },
    ...
    onLogoutButtonTap: function() {
    	if(Core.config.event.Config.getCurrentView()==='eventslide') {      	
    ...
    /**
     * Handles the tap of the new event button. Shows the event detail view.
     */
    ...
    /**
     * Handles the slide disclose of an event slide item. Shows the event detail view passing in a reference to
