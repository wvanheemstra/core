/**
 * The event tile mediator essentially fulfils the passive view pattern for the event tile view.
 */
Ext.define("Core.mediator.touch.event.tile.Mediator", {
...
// CURRENTLY WE DO NOT HAVE A SEARCH IN THE TILE VIEW
    ...
    setupGlobalEventListeners: function() {
        ...
        this.eventBus.addGlobalEventListener(Core.event.event.Event.GET_EVENT_TILE_SUCCESS, this.onGetEventTileSuccess, this);
        this.eventBus.addGlobalEventListener(Core.event.event.Event.GET_EVENT_TILE_FAILURE, this.onGetEventTileFailure, this);
    },

    /**
     * Dispatches the application event to get the tile of events.
     */
    getEventTileData: function() {
        this.logger.debug("getEventTileData");
        this.getView().setMasked({
            xtype: "loadmask",
            message: nineam.locale.LocaleManager.getProperty("eventTile.loading")
        });
        var evt = Ext.create("Core.event.event.Event", Core.event.event.Event.GET_EVENT_TILE);
        this.eventBus.dispatchGlobalEvent(evt);
    },

    /**
     * Handles the show event detail event from the event tile view. Grab the data model
    ...
    showEventDetail: function(record) {
		...
		Core.config.event.Config.setPreviousView('eventtile');
    ...
    /**
     * Handles the login success application-level event. Slide the event tile view
     * onto stage.
     */
    onLoginSuccess: function() {
        this.logger.debug("onLoginSuccess");
		if(Core.config.event.Config.getNextView()==='eventtile') {
        	this.navigate(Core.event.authentication.Event.LOGIN_SUCCESS);
        	this.getEventTileData();
		}
    },
     ...
     */
    onGetEventTileSuccess: function() {
        this.logger.debug("onGetEventTileSuccess");
        this.getView().setMasked(false);
        this.getTile().setStore(this.eventStore);
    },

    /**
     * Handles the get event failure event from the login controller.
     */
    onGetEventTileFailure: function() {
        this.logger.debug("onGetEventTileFailure");
        this.getView().setMasked(false);
    },
    ...
    onLogoutButtonTap: function() {
    	if(Core.config.event.Config.getCurrentView()==='eventtile') {    	
    ...
    /**
     * Handles the tile disclose of a event tile item. Shows the event detail view passing in a reference to
