 ...
 */
Ext.define("Core.mediator.touch.viewport.booking.Mediator", {
    ...
    config: {
		...
		bookingSlideView:        "bookingSlideView",
        bookingTileView:         "bookingTileView",
	...
	setupViewport: function(){
		...
		var background = Core.config.booking.Config.getBackground();
		this.setBackground(background);		
		
		var ui = Core.config.booking.Config.getUi();
		this.setUI(ui);
		
		var company = Core.config.booking.Config.getCompany();
		this.setCompany(company);
		
		var url = Core.config.booking.Config.getUrl();
		this.setURL(url);
		
		var id = Core.config.booking.Config.getId();
		var sessionId = Core.config.booking.Config.getSessionId();
    ...
    navigate: function(action) {
        ...
        switch(action) {      	
            ...
            case Core.event.authentication.Event.LOGOUT_SUCCESS:
				...
				Core.config.booking.Config.setCurrentView('login');
                type = 'pop';
				duration = 600;
				easing = {type: 'ease-out'};
                break;			
				// BOOKING
            case Core.event.navigation.Event.ACTION_SHOW_BOOKING_SLIDE:
                view = this.getBookingSlideView();
				Core.config.booking.Config.setCurrentView('bookingslide');
                direction = 'right';
                break;
				
            case Core.event.navigation.Event.ACTION_SHOW_BOOKING_LIST:
                view = this.getBookingListView();
				Core.config.booking.Config.setCurrentView('bookinglist');
                direction = 'right';
                break;

            case Core.event.navigation.Event.ACTION_SHOW_BOOKING_TILE:
                view = this.getBookingTileView();
				Core.config.booking.Config.setCurrentView('bookingtile');
                direction = 'right';
                break;

            case Core.event.navigation.Event.ACTION_SHOW_BOOKING_MODAL:
                view = this.getBookingModalView();
				Core.config.booking.Config.setCurrentView('bookingmodal');
				type = 'pop';
				duration = 600;
				easing = {type: 'ease-in'};
                break;				
				
            case Core.event.navigation.Event.ACTION_SHOW_BOOKING_DETAIL:
                view = this.getBookingDetailView();
				Core.config.booking.Config.setCurrentView('bookingdetail');
                direction = 'left';
				type = 'slide';
                break;

            case Core.event.navigation.Event.ACTION_BACK_SHOW_BOOKING_SLIDE:
                view = this.getBookingSlideView();
				Core.config.booking.Config.setCurrentView('bookingslide');
                direction = 'right';
				type = 'slide';
            ...
            case Core.event.navigation.Event.ACTION_BACK_SHOW_BOOKING_LIST:
                view = this.getBookingListView();
				Core.config.booking.Config.setCurrentView('bookinglist');
                direction = 'right';
				type = 'slide';
            ...
            case Core.event.navigation.Event.ACTION_BACK_SHOW_BOOKING_TILE:
                view = this.getBookingTileView();
				Core.config.booking.Config.setCurrentView('bookingtile');
                direction = 'right';
				type = 'slide';
            ...
            case Core.event.navigation.Event.ACTION_CLOSE_SHOW_BOOKING_SLIDE:
                view = this.getBookingSlideView();
				Core.config.booking.Config.setCurrentView('bookingslide');
				type = 'pop';
				duration = 600;
