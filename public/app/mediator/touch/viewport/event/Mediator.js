 ...
 */
Ext.define("Core.mediator.touch.viewport.event.Mediator", {
    ...
    config: {
		...
		eventSlideView:         "eventSlideView",
        eventTileView:         "eventTileView",
        eventModalView:		"eventModalView",
	...
	setupViewport: function(){
		...
		var background = Core.config.event.Config.getBackground();
		this.setBackground(background);		
		
		var ui = Core.config.event.Config.getUi();
		this.setUI(ui);
		
		var company = Core.config.event.Config.getCompany();
		this.setCompany(company);
		
		var url = Core.config.event.Config.getUrl();
		this.setURL(url);
		
		var id = Core.config.event.Config.getId();
		var sessionId = Core.config.event.Config.getSessionId();
    ...
    navigate: function(action) {
        ...
        switch(action) {      	
            ...
            case Core.event.authentication.Event.LOGOUT_SUCCESS:
				...
				Core.config.event.Config.setCurrentView('login');
                type = 'pop';
				duration = 600;
				easing = {type: 'ease-out'};
                break;			
				// EVENT
            case Core.event.navigation.Event.ACTION_SHOW_EVENT_SLIDE:
                view = this.getEventSlideView();
				Core.config.event.Config.setCurrentView('eventslide');
                direction = 'right';
                break;
				
            case Core.event.navigation.Event.ACTION_SHOW_EVENT_LIST:
                view = this.getEventListView();
				Core.config.event.Config.setCurrentView('eventlist');
                direction = 'right';
                break;

            case Core.event.navigation.Event.ACTION_SHOW_EVENT_TILE:
                view = this.getEventTileView();
				Core.config.event.Config.setCurrentView('eventtile');
                direction = 'right';
                break;

            case Core.event.navigation.Event.ACTION_SHOW_EVENT_MODAL:
                view = this.getEventModalView();
				Core.config.event.Config.setCurrentView('eventmodal');
				type = 'pop';
				duration = 600;
				easing = {type: 'ease-in'};
                break;				
				
            case Core.event.navigation.Event.ACTION_SHOW_EVENT_DETAIL:
                view = this.getEventDetailView();
				Core.config.event.Config.setCurrentView('eventdetail');
                direction = 'left';
				type = 'slide';
                break;

            case Core.event.navigation.Event.ACTION_BACK_SHOW_EVENT_SLIDE:
                view = this.getEventSlideView();
				Core.config.event.Config.setCurrentView('eventslide');
                direction = 'right';
				type = 'slide';
            ...
            case Core.event.navigation.Event.ACTION_BACK_SHOW_EVENT_LIST:
                view = this.getEventListView();
				Core.config.event.Config.setCurrentView('eventlist');
                direction = 'right';
				type = 'slide';
            ...
            case Core.event.navigation.Event.ACTION_BACK_SHOW_EVENT_TILE:
                view = this.getEventTileView();
				Core.config.event.Config.setCurrentView('eventtile');
                direction = 'right';
				type = 'slide';
            ...
            case Core.event.navigation.Event.ACTION_CLOSE_SHOW_EVENT_SLIDE:
                view = this.getEventSlideView();
				Core.config.event.Config.setCurrentView('eventslide');
				type = 'pop';
				duration = 600;
