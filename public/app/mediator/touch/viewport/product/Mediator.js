 ...
 */
Ext.define("Core.mediator.touch.viewport.product.Mediator", {
    ...
    config: {
		...
		productSlideView:        "productSlideView",
	...
	setupViewport: function(){
		...
		var background = Core.config.product.Config.getBackground();
		this.setBackground(background);		
		
		var ui = Core.config.product.Config.getUi();
		this.setUI(ui);
		
		var company = Core.config.product.Config.getCompany();
		this.setCompany(company);
		
		var url = Core.config.product.Config.getUrl();
		this.setURL(url);
		
		var id = Core.config.product.Config.getId();
		var sessionId = Core.config.product.Config.getSessionId();
    ...
    navigate: function(action) {
        ...
        switch(action) {      	
            ...
            case Core.event.authentication.Event.LOGOUT_SUCCESS:
				...
				Core.config.product.Config.setCurrentView('login');
                type = 'pop';
				duration = 600;
				easing = {type: 'ease-out'};
                break;			
				// PRODUCT
            case Core.event.navigation.Event.ACTION_SHOW_PRODUCT_SLIDE:
                view = this.getProductSlideView();
				Core.config.product.Config.setCurrentView('productslide');
                direction = 'right';
                break;
				
            case Core.event.navigation.Event.ACTION_SHOW_PRODUCT_LIST:
                view = this.getProductListView();
				Core.config.product.Config.setCurrentView('productlist');
                direction = 'right';
                break;

            case Core.event.navigation.Event.ACTION_SHOW_PRODUCT_TILE:
                view = this.getProductTileView();
				Core.config.product.Config.setCurrentView('producttile');
                direction = 'right';
                break;

            case Core.event.navigation.Event.ACTION_SHOW_PRODUCT_MODAL:
                view = this.getProductModalView();
				Core.config.product.Config.setCurrentView('productmodal');
				type = 'pop';
				duration = 600;
				easing = {type: 'ease-in'};
                break;				
				
            case Core.event.navigation.Event.ACTION_SHOW_PRODUCT_DETAIL:
                view = this.getProductDetailView();
				Core.config.product.Config.setCurrentView('productdetail');
                direction = 'left';
				type = 'slide';
                break;

            case Core.event.navigation.Event.ACTION_BACK_SHOW_PRODUCT_SLIDE:
                view = this.getProductSlideView();
				Core.config.product.Config.setCurrentView('productslide');
                direction = 'right';
				type = 'slide';
            ...
            case Core.event.navigation.Event.ACTION_BACK_SHOW_PRODUCT_LIST:
                view = this.getProductListView();
				Core.config.product.Config.setCurrentView('productlist');
                direction = 'right';
				type = 'slide';
            ...
            case Core.event.navigation.Event.ACTION_BACK_SHOW_PRODUCT_TILE:
                view = this.getProductTileView();
				Core.config.product.Config.setCurrentView('producttile');
                direction = 'right';
				type = 'slide';
            ...
            case Core.event.navigation.Event.ACTION_CLOSE_SHOW_PRODUCT_SLIDE:
                view = this.getProductSlideView();
				Core.config.product.Config.setCurrentView('productslide');
				type = 'pop';
				duration = 600;
