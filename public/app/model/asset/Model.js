/**
 * The asset object representing a asset.
 *
 ...
 */
Ext.define("Core.model.asset.Model", Sencha.modelCompatibility({
    extend: "Core.model.asset.base.Model",
	
	// Associated models of hasMany relationships
	requires:[
		"Core.model.asset.group.Model"
	],

	config: {
	    idProperty: "kp_AssetID",
	    fields: [
	        { name: "kp_AssetID", type: "int",	useNull: true },
		...
		hasMany:[
			{
			  name: "AssetGroup",
			  associationKey: "AssetGroup",
			  instanceName: "AssetGroup",
			  getterName: "getAssetGroup",
			  setterName: "setAssetGroup",				  
			  ...
			  foreignStore: "assetGroupStore",  //WAS "Core.store.date.Store", // the store name that contains the related records
			  foreignStoreId: "assetGroupStore" // the store id  of the foreign store
			}		
		],
	    validations: [
	        { type: "presence", field: "kp_AssetID" },
	        { type: "presence", field: "AssetName",     message: "Please enter a name." }
	    ],
		proxy: {
			type: 'ajax',
			url: 'data/?action=write&model=asset&format=json', // Required placeholder		
			actionMethods: {
				create: 'POST',
				write: 'POST',
				read: 'POST',
			...
			api: {
				//read: 'data/read-assets-success.json',
				//read: 'http://api.vanheemstrapictures.com/services/mql/read', // use this in the future when Bluehost allows for this
				read: 'http://localhost:5001/?api=asset&action=read',
				//read: 'http://localhost:5001/services/mql/read',
				//write: 'core/components/core/apps/core/data/mql-single-asset.json',
				write: 'data/?action=write&model=asset&format=json'
			},
			reader: {
				type: 'json',
