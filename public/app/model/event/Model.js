/**
 * The event object representing a event.
 *
 ...
 */
Ext.define("Core.model.event.Model", {
    extend: "Core.model.event.base.Model",
	
	// Associated models of hasMany relationships
	requires:[
		"Core.model.membership.Model",
		"Core.model.event.group.Model"
	],
	
    // Touch uses properties inside of config
	config: {
	    idProperty: "kp_EventID",
	    fields: [
	        { name: "kp_EventID", type: "int",	useNull: true },
		...
		hasMany:[
			{
			  ...
			  name: "EventGroup",
			  associationKey: "EventGroup",
			  instanceName: "EventGroup",
			  getterName: "getEventGroup",
			  setterName: "setEventGroup",				  
			  model: "Core.model.event.group.Model",
			  primaryKey: "kp_EventID", // the field in the parent that identifies it.
			  foreignKey: "kf_EventID", // the key that identifies the parent in the child. In a belongsTo or hasOne relation, this is part of the model itself, in a hasMany relation this is a field of the child objects that refer to my Id.
			  foreignStore: "eventGroupStore",  //WAS "Core.store.date.Store", // the store name that contains the related records
			  foreignStoreId: "eventGroupStore" // the store id  of the foreign store
			}		
		],
	    validations: [
	        { type: "presence", field: "kp_EventID" },
			{ type: "presence", field: "EventName",     message: "Please enter a name." }
	    ],
		proxy: {
			type: 'ajax',
			url: 'data/?action=write&model=event&format=json', // Required placeholder		
			actionMethods: {
				create: 'POST',
				write: 'POST',
				read: 'POST',
			...
			api: {
				//read: 'data/read-events-success.json',
				//read: 'http://api.vanheemstrapictures.com/services/mql/read', // use this in the future when Bluehost allows for this
				read: 'http://localhost:5001/?api=event&action=read',
				//read: 'http://localhost:5001/services/mql/read',
				//write: 'core/components/core/apps/core/data/mql-single-event.json',
				write: 'data/?action=write&model=event&format=json'
			},
			reader: {
				type: 'json',
