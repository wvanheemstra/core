/**
 * The individual object representing a individual.
 *
 ...
 */
Ext.define("Core.model.individual.Model", Sencha.modelCompatibility({
    extend: "Core.model.individual.base.Model",
	
	// Associated models of hasMany relationships
	requires:[
		"Core.model.individual.group.Model"
	],

	config: {
	    idProperty: "kp_IndividualID",
	    fields: [
	        { name: "kp_IndividualID", type: "int",	useNull: true },
	        { name: "IndividualName", type: "string"  },
		...
		hasMany:[
			{
			  ...
			  associationKey: "IndividualGroup",
			  instanceName: "IndividualGroup",
			  getterName: "getIndividualGroup",
			  setterName: "setIndividualGroup",				  
			  ...
			  foreignStore: "individualGroupStore",  //WAS "Core.store.date.Store", // the store name that contains the related records
			  foreignStoreId: "individualGroupStore" // the store id  of the foreign store
			}		
		],
	    validations: [
	        { type: "presence", field: "kp_IndividualID" },
	        { type: "presence", field: "IndividualName",     message: "Please enter a name." }
	    ],
		proxy: {
			type: 'ajax',
			url: 'data/?action=write&model=individual&format=json', // Required placeholder		
			actionMethods: {
				create: 'POST',
				write: 'POST',
				read: 'POST',
			...
			api: {
				//read: 'data/read-individuals-success.json',
				//read: 'http://api.vanheemstrapictures.com/services/mql/read', // use this in the future when Bluehost allows for this
				read: 'http://localhost:5001/?api=individual&action=read',
				//read: 'http://localhost:5001/services/mql/read',
				//write: 'core/components/core/apps/core/data/mql-single-individual.json',
				write: 'data/?action=write&model=individual&format=json'
			},
			reader: {
				type: 'json',
		...
		getHTML: function()
		{
			...
			parentDiv.className = "individual";
			var data = this.data;
			for(field in data){
				if(typeof field !== []){
