/**
 * The slide of booking view for the application.
 *
 * <p> 
 ...
 * </p>
 */
Ext.define("Core.view.touch.booking.slide.View", {
    //extend: "Core.view.touch.booking.base.View",
	extend: "Ext.Container",
    alias: "widget.bookingSlideView",
    controller: "Core.mediator.touch.booking.slide.Mediator",
     ...
     * @param {Core.view.touch.booking.slide.View} this The navigation View instance
						...
						plugins: [
							{
								type: "localization",
								method: "setTitle",
								key: "bookingCategory"+itemId+".title"
							}
						],	
						//
						group: group,
			...
			for (var n = 0; n < apps.length; n++) {
				...
				for (var key in app) {
					...
					if(key == appName || appName == '*'){ // * indicates list all apps
						console.log(value);
						// value is e.g. 
						// {title: "Bookings", url: "http://localhost:4000/touch?app=booking"}
						for (var key in value) {
							console.log(key);
							if(key == 'title'){
								var title = value[key];
						...
						var item = {
							...
							listeners: {
								release: function(button, e, eOpts) {
									...
									var me = button.up('bookingSlideView');
...
//            }
     ...
     *  otherwise this will always return the same thing (the booking container, not the
     *  selected item).
     *
