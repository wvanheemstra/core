/**
 * The slide of event view for the application.
 *
 * <p> 
 ...
 * </p>
 */
Ext.define("Core.view.touch.event.slide.View", {
    //extend: "Core.view.touch.event.base.View",
	extend: "Ext.Container",
    alias: "widget.eventSlideView",
    controller: "Core.mediator.touch.event.slide.Mediator",
     ...
     * @param {Core.view.touch.event.slide.View} this The navigation View instance
						...
						plugins: [
							{
								type: "localization",
								method: "setTitle",
								key: "eventCategory"+itemId+".title"
							}
						],	
						//
						group: group,
			...
			for (var n = 0; n < apps.length; n++) {
				...
				for (var key in app) {
					...
					if(key == appName || appName == '*'){ // * indicates list all apps
						console.log(value);
						// value is e.g. 
						// {title: "Events", url: "http://localhost:4000/touch?app=event"}
						for (var key in value) {
							console.log(key);
							if(key == 'title'){
								var title = value[key];
						...
						var item = {
							...
							listeners: {
								release: function(button, e, eOpts) {
									...
									var me = button.up('eventSlideView');
...
//            }
     ...
     *  otherwise this will always return the same thing (the event container, not the
     *  selected item).
     *
