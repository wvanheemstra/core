/**
 * The slide of organisation view for the application.
 *
 * <p> 
 ...
 * </p>
 */
Ext.define("Core.view.touch.organisation.slide.View", {
    //extend: "Core.view.touch.organisation.base.View",
	extend: "Ext.Container",
    alias: "widget.organisationSlideView",
    controller: "Core.mediator.touch.organisation.slide.Mediator",
     ...
     * @param {Core.view.touch.organisation.slide.View} this The navigation View instance
						...
						plugins: [
							{
								type: "localization",
								method: "setTitle",
								key: "organisationCategory"+itemId+".title"
							}
						],	
						//
						group: group,
			...
			for (var n = 0; n < apps.length; n++) {
				...
				for (var key in app) {
					...
					if(key == appName || appName == '*'){ // * indicates list all apps
						console.log(value);
						// value is e.g. 
						// {title: "Organisations", url: "http://localhost:4000/touch?app=organisation"}
						for (var key in value) {
							console.log(key);
							if(key == 'title'){
								var title = value[key];
						...
						var item = {
							...
							listeners: {
								release: function(button, e, eOpts) {
									...
									var me = button.up('organisationSlideView');
...
//            }
     ...
     *  otherwise this will always return the same thing (the organisation container, not the
     *  selected item).
     *
