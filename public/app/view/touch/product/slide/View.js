/**
 * The slide of product view for the application.
 *
 * <p> 
 ...
 * </p>
 */
Ext.define("Core.view.touch.product.slide.View", {
    //extend: "Core.view.touch.product.base.View",
	extend: "Ext.Container",
    alias: "widget.productSlideView",
    controller: "Core.mediator.touch.product.slide.Mediator",
     ...
     * @param {Core.view.touch.product.slide.View} this The navigation View instance
						...
						plugins: [
							{
								type: "localization",
								method: "setTitle",
								key: "productCategory"+itemId+".title"
							}
						],	
						//
						group: group,
			...
			for (var n = 0; n < apps.length; n++) {
				...
				for (var key in app) {
					...
					if(key == appName || appName == '*'){ // * indicates list all apps
						console.log(value);
						// value is e.g. 
						// {title: "Products", url: "http://localhost:4000/touch?app=product"}
						for (var key in value) {
							console.log(key);
							if(key == 'title'){
								var title = value[key];
						...
						var item = {
							...
							listeners: {
								release: function(button, e, eOpts) {
									...
									var me = button.up('productSlideView');
...
//            }
     ...
     *  otherwise this will always return the same thing (the product container, not the
     *  selected item).
     *
