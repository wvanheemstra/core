[[$core.HeaderTpl]]
   <div id="page" style="margin-left:10px;margin-right:10px;">
		...
		<!-- END Optional breadcrumbs -->
                ...
                <!-- START Edit article -->
                <div class="row">
                  <div id="edit_article" class="twelve columns">
		[[!Profile]]
		[[!FormItRetriever? 
...
[[!If?
  ...
  &operator=`equal`
  &operand=`1`
  &then=`<th><h5>[[%custom.label_article_removed? &namespace=`custom`]]:</h5></th><th><h5 style="color:red;">[[+fi.pagetitle]]</h5></th>`
  &else=`<th><h5>[[%custom.label_article_edited? &namespace=`custom`]]:</h5></th><th><h5>[[!getResources? &resources=`[[+fi.resource_id]]` &tpl=`core.ArticleEditedRowTpl`]]</h5></th>`
]]
  </tr>
</thead>
    <input name="publishedon" type="hidden" value="[[+fi.publishedon]]" />
    <input name="publishedby" type="hidden" value="[[+fi.publishedon]]" />
    <input name="editedon" type="hidden" value="[[+fi.editedon]]" />
    <input name="editedby" type="hidden" value="[[+fi.editedby]]" />
    <input name="createdon" type="hidden" value="[[+fi.createdon]]" />
    <input name="createdby" type="hidden" value="[[+fi.createdby]]" />
    <input name="resource_id" type="hidden" value="[[+fi.resource_id]]" />
<tbody>
  <tr>
    <td>
...
[[!Wayfinder? &startId=`[[++custom.site_start]]` &startIdContext=`[[++custom.context]]` &ignoreHidden=`1` &where=`{"class_key:=": "ArticlesContainer"}` &rowTpl=`core.EditArticleParentRowTpl` &outerTpl=`core.EditArticleParentOuterTpl` &parentRowTpl=`core.EditArticleParentParentRowTpl` &sortBy=`menutitle` &sortOrder=`ASC` &includeDocs=`[[++custom.include_docs_as_article_parent]]`]]
    </select>
    </td>
  </tr><tr>
    <td>     
