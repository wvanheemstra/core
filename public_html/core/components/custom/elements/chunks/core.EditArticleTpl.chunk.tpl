[[$core.HeaderTpl]]
   <div id="page">
		...
		&redirectTo=`[[++custom.article_edited]]`
		&store=`1`
		...
		&resource2formitfields=`resource_id,publishedon,publishedby,editedon,editedby,BreadCrumbTV,parent,pagetitle,menutitle,longtitle,description,introtext,content,SideBarTV,SideContentTV,TwitterFeedTV,LatestPostsTV,LatestCommentsTV,NumberOfPageBoxesTV,PageBox01TV,PageBox02TV,PageBox03TV,PageBox04TV,SubpagesTV,published,isfolder,deleted`
		&successMessage=`[[%custom.label_article_edited? &namespace=`custom`]].`
...
<form id="edit_article_form" class="form custom nice" action="[[~[[*id]]]]" method="post">

    <h5>[[%custom.label_edit_article? &namespace=`custom`]]: <a href="[[+fi.url]]">[[+fi.pagetitle]]</a></h5>
    ...
    <input name="pub_date" type="hidden" value="[[+fi.pub_date]]" />
    <input name="publishedby" type="hidden" value="[[+fi.publishedby]]" />
    <input name="editedon" type="hidden" value="[[!getDateTimeStamp]]" />
    <input name="editedby" type="hidden" value="[[+modx.user.id]]" />
    <input name="createdon" type="hidden" value="[[+fi.createdon]]" />
    <input name="createdby" type="hidden" value="[[+fi.createdby]]" />
...
[[!Wayfinder? &startId=`[[++custom.site_start]]` &startIdContext=`[[++custom.context]]` &ignoreHidden=`1` &where=`{"class_key:=": "ArticlesContainer"}`&rowTpl=`core.EditArticleParentRowTpl`  &outerTpl=`core.EditArticleParentOuterTpl` &innerTpl=`core.EditArticleParentInnerTpl` &parentRowTpl=`core.EditArticleParentParentRowTpl` 
&sortBy=`menutitle` &sortOrder=`ASC` &includeDocs=`[[++custom.include_docs_as_article_parent]]`]]
    </select>
		</td>
		</tr>
	</tbody>
...
<div class="form-buttons">
        <input class="nice small white button" type="submit" value="[[%custom.label_edit_article? &namespace=`custom`]]" />
    </div>
</form>
                  </div><!-- #edit_article -->
                </div>
   ...
   <script type="text/javascript" >
      $('edit_article_form').bind('form-pre-serialize', function(e) {
          tinyMCE.triggerSave();
      });
   </script >
[[$core.FooterTpl]]